...
Create separate environments for clair3 and medaka and use conda/mamba run-n clair3/medaka to use them
The example section is as follows

```config
# cfgfile
...
phase_clair3_command=mamba run -n clair3 run_clair3.sh
polish_medaka_command=mamba run -n medaka medaka
...
