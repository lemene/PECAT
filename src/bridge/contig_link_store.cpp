...
void ContigLinkStore::LoadR2cFile(const std::string &fname) {
    ...
    // 计算ctg的平均Coverage
    for (auto &ctg : read2ctg_group_) {
        std::vector<int> cov;
        for (auto &rd : ctg.second) {
            const auto o = rd.second;
            if (cov.size() == 0) {
                cov.assign(o->b_.len+1, 0);
            }
...
void ContigLinkStore::LoadC2cFile(const std::string &fname) {
    ...
    for (size_t i=0; i < ctg2ctg_.Size(); ++i) {
        const auto &o = ctg2ctg_.Get(i);
...
void ContigLinkStore::Dump(const std::string &fname) {
    ...
    if (of != nullptr) {
        for (auto &ctg0 : links_) {
            for (auto &ctg1 : ctg0.second) {
                auto& link = ctg1.second;
                gzprintf(of, "\n%d %s %d %s\n", ctg0.first, read_store_.QueryNameById(ctg0.first).c_str(), ctg1.first, read_store_.QueryNameById(ctg1.first).c_str());
                //of << ctg0.first << " " << read_store_.QueryNameById(ctg0.first) << " " << ctg1.first << " " << read_store_.QueryNameById(ctg1.first) << "\n";
                gzprintf(of, "Contig_Contig\n");
                //of << "Contig_Contig\n";
                for (auto &i : link.c2c_links) {
                    for (auto& o : i.ols) {
                        gzprintf(of, "%s\n", ctg2ctg_.ToM4aLine1(*o).c_str());
