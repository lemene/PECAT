...
std::vector<Seq::Tile> ContigLink::C2r2cLink::GetSeqTiles(Seq::Id id, char end) const {
    if (target.id == id) {
        ...
        if (end == 'B') {
            ...
            if (s >= target.len) {
                ...
                if (t2r[0] < t2r[1]) {
                    return { Seq::Tile{ ols[1]->a_.id, 0, t2r[0] , t2r[1] },  Seq::Tile{ id, 1, 0, target.len } };
                }
                else {
                    return { Seq::Tile{ ols[1]->a_.id, 1, t2r[1], t2r[0] },  Seq::Tile{ id, 1, 0, target.len } };
                }
            }
            else {

                return { Seq::Tile{ id, 1, 0, s } };
            }
        }
        else {
            assert(end == 'E');
            ...
            if (e < 0) {
                ...
                if (t2r[0] < t2r[1]) {
                    return { Seq::Tile{ ols[1]->a_.id, 0, t2r[0] , t2r[1] },  Seq::Tile{ id, 0, 0, target.len } };
                }
                else {
                    return { Seq::Tile{ ols[1]->a_.id, 1, t2r[1], t2r[0] },  Seq::Tile{ id, 0, 0, target.len } };
                }
            }
            else {

                return { Seq::Tile{ id, 0, e, target.len } };
            }
        }
    }
    else {
        ...
        if (end == 'B') {
            ...
            if (s >= source.len) {
                ...
                if (t2r[0] < t2r[1]) {
                    return { Seq::Tile{ ols[0]->a_.id, 0, t2r[0] , t2r[1] },  Seq::Tile{ id, 1, 0, source.len } };
                }
                else {
                    return { Seq::Tile{ ols[0]->a_.id, 1, t2r[1], t2r[0] },  Seq::Tile{ id, 1, 0, source.len } };
                }
            }
            else {
            ...
            if (e < 0) {
                ...
                if (t2r[0] < t2r[1]) {
                    return { Seq::Tile{ ols[0]->a_.id, 0, t2r[0] , t2r[1] },  Seq::Tile{ id, 0, 0, source.len } };
                }
                else {
                    return { Seq::Tile{ ols[0]->a_.id, 1, t2r[1], t2r[0] },  Seq::Tile{ id, 0, 0, source.len } };
                }
            }
            else {

                return { Seq::Tile{ id, 0, e, source.len } };
            }
        }
    }
}
...
std::vector<Seq::Tile> ContigLink::C2cLink::GetSeqTiles(Seq::Id id, char end) const {
    if (target.id == id) {
        if (end == 'B') {
            return { Seq::Tile{id, 1, 0, target.start} };
        }
        else {
            assert(end == 'E');

            return { Seq::Tile{ id, 0, target.end, target.len } };
        }
    }
    else {
        if (end == 'B') {
            return { Seq::Tile{ id, 1, 0, source.start } };
        }
        else {
            assert(end == 'E');

            return { Seq::Tile{ id, 0, source.end, source.len } };
        }
    }
}
...
bool ContigLink::SimpleValid(const Overlap &s, const Overlap &t, int err) {
    ...
    return loc_s != Overlap::Loc::Abnormal && loc_t != Overlap::Loc::Abnormal && (
           ...
           (loc_s == Overlap::Loc::Equal || loc_s == Overlap::Loc::Contained || loc_s == Overlap::Loc::Containing) ||
           (loc_t == Overlap::Loc::Equal || loc_t == Overlap::Loc::Contained || loc_t == Overlap::Loc::Containing));
}
