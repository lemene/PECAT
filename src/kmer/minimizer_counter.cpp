...
std::vector<Minimizer> MinimizerCounter::Count(const DnaSeq& seq, const RankedKmers& rkmers) {
    ...
    auto to_minimizer = [mask, &rkmers](const std::array<KmerId, 2>& kmer, size_t pos, size_t rid) -> Minimizer {
        Minimizer m;
    ...
    //     LOG(INFO)("kmer %zd %016llX", i, mm.hash);
    // }
    ...
    //     LOG(INFO)("mer: %zd %zd %016llX", i, minimizers[i].pos, minimizers[i].hash);
    // }
    return minimizers;
}
